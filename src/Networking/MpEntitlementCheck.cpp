...
#pragma endregion
    ...
    static void HandleEntitlementReceived(StringW userId, StringW levelId, EntitlementsStatus entitlement) {
        ...
        DEBUG("[HandleEntitlementReceived] Received Entitlement from user '{}' for level '{}' with status '{}'",
            cUserId,
            cLevelId),
            EnumUtils::GetEnumName(entitlement);
...
#pragma region Hooks
    MAKE_HOOK_MATCH(NetworkPlayerEntitlementChecker_GetEntitlementStatus, &NetworkPlayerEntitlementChecker::GetEntitlementStatus, Task_1<EntitlementsStatus>*, NetworkPlayerEntitlementChecker* self, StringW levelIdCS) {
        ...
        INFO("NetworkPlayerEntitlementChecker_GetEntitlementStatus: {}", levelId);
        if (IsCustomLevel(levelId)) {
            if (HasSong(levelId)) {
                INFO("NetworkPlayerEntitlementChecker_GetEntitlementStatus has song, checking requirements");
                ...
                INFO("NetworkPlayerEntitlementChecker_GetEntitlementStatus we don't have the song, falling back to beatsaver compare");
                                    ...
                                    if (mapHash == Utilities::GetHash(levelId)) {
                                        ...
                                        for (auto& diff : beatmap.GetDiffs()) {
                                            ...
                                            /*else*/ if (diff.GetNE() && !RequirementUtils::GetRequirementInstalled("Noodle Extensions") && !RequirementUtils::GetIsForcedSuggestion("Noodle Extensions")) {
                                                ...
                                                task->TrySetResult(EntitlementsStatus::NotOwned);
                                                WARNING("Map contains ME difficulty but MappingExtensions doesn't seem to be installed, returning NotOwned");
                                        ...
                                        ERROR("Hash returned by BeatSaver doesn't match requested hash: Expected: '{}' got '{}'", Utilities::GetHash(levelId).c_str(), mapHash.c_str());
    ...
    void HandleNetworkPlayerEntitlementCheck_Start(NetworkPlayerEntitlementChecker* entitlementChecker) {
        if (!entitlementAction) {
            entitlementAction = il2cpp_utils::MakeDelegate<System::Action_3<::StringW, ::StringW, EntitlementsStatus>*>(classof(System::Action_3<::StringW, ::StringW, EntitlementsStatus>*), &HandleEntitlementReceived);
        }
    ...
    void HandleNetworkPlayerEntitlementCheck_OnDestroy(NetworkPlayerEntitlementChecker* entitlementChecker) {

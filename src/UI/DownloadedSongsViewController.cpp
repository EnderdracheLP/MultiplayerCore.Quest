...
DEFINE_TYPE(MultiplayerCore::UI, DownloadedSongsViewController);

namespace MultiplayerCore::UI {
    bool cellIsSelected = false;
    DownloadedSongsViewController* DownloadedSongsViewController::instance;
    std::vector<std::string> DownloadedSongsViewController::mapQueue;

    void DownloadedSongsViewController::CreateCell(System::Threading::Tasks::Task_1<UnityEngine::Sprite*>* coverTask, CustomPreviewBeatmapLevel* level) {
    ...
    void DownloadedSongsViewController::Delete() {
        try {
    ...
    void DownloadedSongsViewController::DidActivate(bool firstActivation, bool addedToHierarchy, bool screenSystemEnabling) {
        getLogger().debug("DownloadedSongsViewController::DidActivate");
        if (firstActivation) {
            instance = this;

            modal = BeatSaberUI::CreateModal(get_transform(), { 55, 35 }, [this](HMUI::ModalView* self) {
                list->tableView->ClearSelection();
                });
            ...
            list = BeatSaberUI::CreateScrollableList(vertical->get_transform(), { 80, 67 }, [this](int idx) {
    ...
    void DownloadedSongsViewController::InsertCell(std::string hash) {
    ...
    void DownloadedSongsViewController::InsertCell(GlobalNamespace::CustomPreviewBeatmapLevel* level) {
        getLogger().debug("DownloadedSongsGSM::InsertCell");
        if (level) {
    ...
    void DownloadedSongsViewController::Refresh() {
        list->tableView->ReloadData();
        list->tableView->RefreshCellsContent();
    }

    void DownloadedSongsViewController::OnEnable() {
        if (list && list->tableView) {
            if (!mapQueue.empty()) {
                InsertCell(mapQueue.back());
                mapQueue.pop_back();
